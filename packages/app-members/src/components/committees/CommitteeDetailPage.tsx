...
export function CommitteeDetailPage() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      navigate("/admin/meetings/committees");
    } finally {
      setDeleting(false);
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <Button variant="ghost" size="icon" asChild>
              <Link to="/admin/meetings/committees">
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {membersToShow.map((member) => (
                  <div key={member.id} className="relative group">
                    <MemberCard
                      member={member}
                      onNavigate={(memberId) => navigate(`/admin/members/${memberId}`)}
        ...
        <CardHeader className="flex flex-row items-center justify-between space-y-0">
          ...
          {committee.status === "active" && (
            <Button asChild variant="outline" size="sm">
              <Link to={`/admin/meetings/committees/${id}/meetings/new`}>
                <Plus className="size-4" />
                New meeting
              </Link>
            </Button>
              ...
              <tbody>
                {(committee.meetings ?? []).map((m) => (
                  <tr key={m.id} className="border-b last:border-0">
                    <td className="py-2">
                      <Link
                        to={`/admin/meetings/committees/${id}/meetings/${m.id}`}
                    ...
                    <td className="py-2 text-right">
                      <Button variant="ghost" size="sm" asChild>
                        <Link to={`/admin/meetings/committees/${id}/meetings/${m.id}`}>View</Link>
                      </Button>
                    </td>
                  </tr>
                ))}
