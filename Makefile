...
help:
	...
	@printf '  $(GRN)build-static$(RST)     Build unified static site (app-public + app-members) into $(STATIC_DIR)/\n'
...
build-static:
	@printf '$(CYN)Building static site (app-public + app-members) -> $(STATIC_DIR)/$(RST)\n'
	...
	@cd packages/app-members && OUTDIR=../../$(STATIC_DIR)/admin API_ORIGIN="$(API_ORIGIN)" bun run build
